...
export const log = {
  ...
  warn: (msg: string) => console.error(`${colors.yellow}[warn]${colors.reset} ${msg}`),
...
export function successResult<T>(action: string, data: T, message: string, nextSteps?: string[]): Result<T> {
  return { success: true, action, data, message, nextSteps };
}

export function errorResult(action: string, error: string, message: string): Result {
...
export async function listWorktrees(): Promise<Array<{ path: string; branch: string; bare: boolean }>> {
  try {
...
export async function runPackageManagerInstall(dir: string, pm?: PackageManager): Promise<void> {
