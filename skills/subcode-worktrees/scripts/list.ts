...
async function getWorktrees(): Promise<
...
async function main() {
	...
	const worktrees: WorktreeInfo[] = await Promise.all(
		rawWorktrees.map(async (wt) => {
			...
			const relativePath = isMain
				? repoRoot
				: relative(repoRoot, wt.path) || wt.path;
	...
	if (jsonOutput) {
		...
		if (worktrees.length === 0) {
			...
			console.log("");
			console.log(
				`Total: ${worktrees.length} worktrees (${subcodeCount} in .subcode/worktrees/)`,
			);
		}
	}
}
